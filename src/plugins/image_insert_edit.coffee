$.widget "ncri.hallo-image-insert-edit",
  ...
  populateToolbar: ($toolbar) ->
    ...
    $( @options.editable.element ).on "click", "img", (e) ->
  ...
  _openDialog: ($image) ->
    ...
    if @$image
      yposition = @$image.offset().top - $(document).scrollTop()
    else
  ...
  _load_dialog_image_properties_ui: ->
    ...
    if @$image
      $('#insert_image_btn').remove() unless @options.insert_file_dialog_ui_url

      if $('#hallo_img_file_select_title').length > 0
        $('#hallo_img_file_select_title').text @texts.chage_image

      $('#hallo_img_properties #hallo_img_source').keyup ->
        widget.$image.attr 'src', this.value

      $('#hallo_img_properties #hallo_img_alt').keyup ->
        widget.$image.attr 'alt', this.value

      $('#hallo_img_properties #hallo_img_padding').keyup ->
        widget.$image.css 'padding', this.value

      $('#hallo_img_properties #hallo_img_height').keyup ->
        widget.$image.css 'height', this.value
        widget.$image.attr 'height', this.value

      $('#hallo_img_properties #hallo_img_width').keyup ->
        widget.$image.css 'width', this.value
        widget.$image.attr 'width', this.value

      $('#hallo_img_properties #hallo_img_float_left').click ->
      ...
      $('#hallo_img_properties #hallo_img_float_right').click ->
        return false unless this.checked
        widget.$image.css 'float', 'right'
        $('#hallo_img_properties #hallo_img_unfloat').removeAttr('checked')
        $('#hallo_img_properties #hallo_img_float_left').removeAttr('checked')

      $('#hallo_img_properties #hallo_img_unfloat').click ->
        return false unless this.checked
